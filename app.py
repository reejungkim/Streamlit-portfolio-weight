...
start_date = st.sidebar.date_input(
    'Start date', today - dt.timedelta(days=100))
...
if (tickers_selected != []):
    ...
    for i in tickers_selected:
        ...
        try:
            symbol_data = pdr.DataReader(
                i, 'yahoo', start_date, end_date).reset_index()
            # display(symbol_data)
        except (KeyError, ValueError):  # the error could possibly occur when there's "." in stock name
            symbol_data = pdr.DataReader(
                i.replace('.', '-'), 'yahoo', start_date, end_date).reset_index()
            ...
            symbol_data = pd.DataFrame()
            pass
        # axis=0 <- row. add frames by row and use fill down.
        single_table = pd.concat(
            [symbol, symbol_data], axis=0, ignore_index=True)
...
# GRAPH
    ...
    d = df.pivot_table(values=price_indicator, index='Date',
                       columns='ticker', aggfunc=np.sum, margins=False)
    ...
    st.write(
        'expected annual return based on selected range of historical data', avg_return)
...
# HIGHEST SHARP RATIO
    ...
    opts = sco.minimize(min_func_sharpe, len(tickers_selected) * [1. / len(
        tickers_selected), ], method='SLSQP', bounds=bnds, constraints=cons)
    ...
    st.write('Weight of portfolio that returns highest sharp ratio',
             optimal_weights_sharp)
    pt_opts = statistics(opts['x']).round(3)
    plt.scatter(pt_opts[1], pt_opts[0], marker="*",
                s=500, alpha=0.5, color='black')

    st.write("exp return :" + str(statistics(opts['x'].round(3))[0].round(3)))
    st.write("exp volatility :" +
             str(statistics(opts['x'].round(3))[1].round(3)))
    st.write("exp sharp ratio :" +
             str(statistics(opts['x'].round(3))[2].round(3)))
...
# MINIMUM VARIANCE
    ...
    plt.plot(pt_optv[1], pt_optv[0], marker="*",
             markersize=20, alpha=0.5, color='black')
...
# EFFICIENT FRONTIER
    ...
    plt.scatter(tvols, trets, c=trets / tvols, marker='x',
                s=70, linewidth=2, cmap='plasma')

    # Efficient frontier
    ind = np.argmin(tvols)
    # include array only up to the index of minimum volatility
    evols = tvols[ind:]
    erets = trets[ind:]
